...
//     spgemm(A, B, C, MatrixTypeA::format(), MatrixTypeB::format(),
//     MatrixTypeC::format());
// }

bool equalZeros(int a) { return a == 0; }
...
void spgemm(csr_matrix<IndexType, ValueType> &A,
            csc_matrix<IndexType, ValueType> &B,
...
std::vector<node> merge_with_reduce(std::vector<node> &l1,
                                    std::vector<node> &l2, Compare cmp) {
...
template <typename IndexType, typename ValueType>
std::pair<double, double> outer_spgemm(csc_matrix<IndexType, ValueType> &A,
                                       csr_matrix<IndexType, ValueType> &B,
                                       csr_matrix<IndexType, ValueType> &C) {
  struct node {
    IndexType index;
    ValueType value;
  ...
  for (size_t i = 0; i < A.num_cols; ++i) {
    ...
    if (A_ith_col_number == 0 || B_ith_row_number == 0)
      continue;
...
outer_spgemm_with_condensing(csr_matrix<IndexType, ValueType> &A,
                             csr_matrix<IndexType, ValueType> &B,
                             csr_matrix<IndexType, ValueType> &C) {
  struct node {
    IndexType index;
  ...
  for (size_t round_index = 0; round_index < A.num_cols; ++round_index) {
    for (size_t col_index = 0; col_index < ROUND_WIDTH; ++col_index) {
      ...
      for (size_t i = 0; i < A.num_rows; ++i) {
        if (A.row_offsets[i + 1] - A.row_offsets[i] >
            now_condensing_col_index) {
          now_col_value.push_back(
              A.values_array[A.row_offsets[i] + now_condensing_col_index]);
          now_col_index.push_back(
              A.column_indices[A.row_offsets[i] + now_condensing_col_index]);
      ...
      // std::vector<size_t> order = ftxj::estd::sort_indexes(&now_col_index[0],
      // now_col_index.size());
      for (size_t i = 0; i < now_col_index.size(); ++i) {
        ...
        for (size_t j = 0; j < number; ++j) {
          ValueType c =
              now_col_value[i] * B.values_array[B.row_offsets[item_col] + j];
        ...
        if (temp_matrixs[aindex].size() != 0) {
          temp_vec =
              merge_with_reduce(temp_vec, *temp_matrixs[aindex].begin(), cmp);
        }
