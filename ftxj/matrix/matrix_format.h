...
public:
  ...
  base_matrix() : num_rows(0), num_cols(0), num_entries(0) {}
...
class coo_matrix : public base_matrix<IndexType, ValueType, coo_format> {
  ...
  coo_matrix(IndexType *rows, IndexType *colums, ValueType *values, size_t r,
             size_t c, size_t nnzs) {}
};
...
public:
  ...
  csr_matrix(size_t num_rows, size_t num_cols, size_t num_entries)
      : base_matrix(num_rows, num_cols, num_entries) {}
  csr_matrix(IndexType *offsets, IndexType *colums, ValueType *values,
             size_t rows, size_t cols, size_t nnzs) {
  ...
  csr_matrix(IndexType *rows_start, IndexType *rows_end, IndexType *colums,
             ValueType *values, size_t rows, size_t cols) {
    IndexType nnzs = 0;
    row_offsets.push_back(0);
    for (size_t i = 0; i < rows; ++i) {
      size_t row_ith_number = rows_end[i] - rows_start[i];
      std::vector<size_t> v =
          ftxj::estd::sort_indexes(&colums[rows_start[i]], row_ith_number);
  ...
  bool equals(csr_matrix<IndexType, ValueType> &b) {
    ...
    for (size_t i = 0; i < this->num_rows; ++i) {
      if (values_array[i] != b.values_array[i] ||
          column_indices[i] != b.column_indices[i] ||
...
public:
  ...
  csc_matrix(size_t num_rows, size_t num_cols, size_t num_entries)
      : base_matrix(num_rows, num_cols, num_entries) {}
  csc_matrix(IndexType *offsets, IndexType *colums, ValueType *values,
             size_t rows, size_t cols, size_t nnzs) {}
};
