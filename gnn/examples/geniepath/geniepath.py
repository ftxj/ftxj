...
class Breadth(torch.nn.Module):
    def __init__(self, in_dim, out_dim):
        super().__init__()
        self.gatconv = GATConv(in_dim, out_dim, heads=1).jittable()
...
class GeniePathLazy(torch.nn.Module):
    def __init__(self, in_dim, out_dim):
        ...
        self.depths = torch.nn.ModuleList(
            [Depth(dim * 2, 256) for i in range(layer_num)])
        self.lin2 = torch.nn.Linear(dim, out_dim)
    ...
    def forward(self, x, edge_index):
        ...
        for breadth in self.breadths:
            h_tmps.append(breadth(x, edge_index))
        x = x[None, :]
        i = 0
        for dep in self.depths:

            in_cat = torch.cat((h_tmps[i][None, :], x), -1)
            x, (h, c) = dep(in_cat, h, c)
            i = i + 1
        x = self.lin2(x[0])
        return x
