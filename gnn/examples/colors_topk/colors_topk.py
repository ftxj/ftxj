...
class Net(torch.nn.Module):
    ...
    def forward(self, x, edge_index, batch, attn):
        ...
        attn_loss = F.kl_div(torch.log(score + 1e-14), attn[perm],
                             reduction='none')
...
def train(epoch):
    ...
    for data in train_loader:
        data = data.to(device)
        optimizer.zero_grad()
        out, attn_loss, _ = model(data.x, data.edge_index, data.batch, data.attn)
...
def test(loader):
    ...
    for data in loader:
        data = data.to(device)
        out, _, ratio = model(data.x, data.edge_index, data.batch, data.attn)
