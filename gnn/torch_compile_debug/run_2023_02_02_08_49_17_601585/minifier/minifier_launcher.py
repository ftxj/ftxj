...
class Repro(torch.nn.Module):
    ...
    def forward(self, edge_type : torch.Tensor, edge_index_j : torch.Tensor, self_weight : torch.nn.parameter.Parameter, self_comp : torch.nn.parameter.Parameter):
        view = self_weight.view(30, -1);  self_weight = None
        matmul = self_comp @ view;  self_comp = view = None
        view_1 = matmul.view(90, 6909, 16);  matmul = None
        mul = edge_type * 6909;  edge_type = None
        add = mul + edge_index_j;  mul = edge_index_j = None
        view_2 = view_1.view(-1, 16);  view_1 = None
        getitem = view_2[add];  view_2 = add = None
        return (getitem,)
